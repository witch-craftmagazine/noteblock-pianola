...
Renders each MIDI headlessly through SpessaSynth, measures RMS loudness, and writes per-track gain multipliers to `volumeAdjustments.json` (ReplayGain-style normalization). Requires `minecraft3.sf2` to be present at the repo root — **not run in CI** because the soundfont is too large for the repo. Incremental by default.
...
**Not in CI:** `analyze-loudness.js` requires `minecraft3.sf2` which is not stored in the repo. Run it locally via `node tools/sync.js` and commit `volumeAdjustments.json` manually when adding new tracks.
